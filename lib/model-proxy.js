...
class ModelProxy extends Engine {
  constructor(interfaces, option = {}) {
    ...
    const proxy = (id, opts) => {
      ...
      const request = Object.assign({}, model, opts); // 请求对象

      if (!engines.has(request.engine)) {
        throw Error(`没有发现 ${request.engine} 引擎！`);
      }
      ...
      if (request.params) {
        Object.keys(request.params).forEach((key) => {
          url = url.replace(new RegExp(`:${key}\\b`, 'g'), request.params[key]);
        });
      }

      const ctx = { id, name, url, model, request, option }; // 上下文对象
      const engine = engines.get(request.engine); // 获取引擎
